...
var gitMech = {
    ...
    childProcess.exec(this.gitBin + ' --version', function (err, stdout, stderr) {
      ...
      try {
        ...
        if (process.platform === 'win32') {
          ...
          gitDir = fswin.convertPathSync(gitDir, false)
          workTree = fswin.convertPathSync(workTree, false)
        }
  ...
  ls: function (pattern, callback) {
    var lsCommands = [ 'ls-tree', '--name-only', '-r', 'HEAD' ]
    if (process.platform === 'win32') {
      lsCommands = lsCommands.concat(['|findstr /I "' + pattern.replace(/^\*/, '') + '"'])
    } else {
      lsCommands = lsCommands.concat([ '--', docSubdir + pattern ])
    }
    gitExec(lsCommands, function (err, data) {
      if (err) {
        data = ''
      }
